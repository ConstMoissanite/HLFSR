...
16-bit `idx_init` 全部利用：
- **低 9 位**（0--511）：起始步游标，确定坐标解码的初始面/行/列
- **高 7 位**（0--127）：矩阵搅拌参数，在 64 步预热前线性扩散到全 64 字节（`extra + i × 0xB9`）

相同 `key_material` + 不同高 7 位 → 128 种不同初始矩阵状态，等效扩展密钥空间。
