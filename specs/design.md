...
init(key_material[64], idx_init(u16)):
  ...
  5. 运行 512 步 next() 并丢弃输出  ← 启动混合 (见下文)
```

**设计要点**：
- 64 字节统一初始化 matrix 和 LFSR，无分离种子
- **启动混合 (512 步预热)**：init 完成后立即运行 512 步 next() 丢弃输出。对齐 Trivium 级别。原因：
  - 未经预热时第一步 mask=km[0]，参与 XOR 的 LFSR 状态 = km[0*8..7]，输出直接暴露原始密钥材料
  - 512 步 = 矩阵 64 字节完整遍历 8 轮 (与 col 的 64 步周期无关)
  - 第 257 步输出依赖深度混洗后的状态，差分 avalanche ~29%→49–50%，代数度 <7→≥8
  - 预热开销 ~2µs，吞吐无影响
