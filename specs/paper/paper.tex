...
ChaCha20 implementation and 2.2$\times$ more area-efficient than ChaCha20 in
...
\midrule
Gates (Kgate)          & 23 & 20 & 25 \\
Frequency (GHz)        & 0.8 & 0.4 & 3.0 \\
Throughput (GB/s)      & 6.4 & 2.5 & 10 \\
\cmidrule(lr){1-4}
\textbf{Normalized} \\
\quad Area eff.\ (Gbps/Kgate) & \textbf{0.28} & 0.125 & 0.40 \\
\quad Arch.\ eff.\ (GB/s/GHz) & \textbf{8.0} & 6.25 & 3.33 \\
\quad Gate cost (Kgate/Gbps)  & \textbf{3.6} & 8.0 & 2.5 \\
\bottomrule
\end{tabular}
...
\textbf{Gate-count breakdown} (pre-synthesis, module-level estimates):
64$\times$64 Dadda multiplier $\approx$ 18K gates (4096 partial-product ANDs
+ $\sim$2000 carry-save adders in reduction tree + 128-bit CLA); 8 Galois
LFSRs (64b registers + XOR + shift logic) $\approx$ 4K gates; 64-byte
matrix register file + address decode $\approx$ 4K gates; feedback constant
multiplier (16$\times$64$\to$16-bit, K$_2$ constant-optimized) $\approx$
2.5K gates; control FSM $\approx$ 0.5K gates. Subtotal $\approx$ 29K gates;
independent parallel evaluation suggests $\approx$ 23K gates --- we adopt
the more conservative 23K estimate. Frequency based on multiplier critical
path (Dadda reduction $\sim$3 stages + CLA $\sim$5 stages, $\approx$ 8--10
FO4 $\approx$ 1.2--1.5ns @ 7nm), empirically derated to 800 MHz. None of
these figures have been validated by logic synthesis; $\pm$30\% expected.

After normalization, HLFSR-64 is comparable to AES-128 in area efficiency
(0.28 vs.\ 0.40 Gbps/Kgate) and significantly ahead of ChaCha20 (0.125).
The dominant area cost is the 64$\times$64 full multiplier (60--78\% of
total gates); replacing it with a pipelined smaller multiplier or iterative
multiply-accumulate architecture would substantially reduce area.
