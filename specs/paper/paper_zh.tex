...
\begin{abstract}
HLFSR-64是一种面向软件性能优化的流密码，采用$8\times8\times8$位反馈矩阵、8条64位Galois LFSR和16位步计数器（低9位计数+高7位存反馈态）作为状态。每步推进全部LFSR后全异或得$v_x$，经$\mathrm{ROTL}_{33}$自旋转与掩码派生的奇数乘子级联相乘产生$\mathsf{raw}$。256位输出通过单次额外乘法派生：LFSR与矩阵状态交叉，经动态调节后乘$K_3$，以不同旋转量生成4路乘子分别与$\mathsf{raw}$相乘，各路异或不同LFSR值消除派生相关性。反馈为$\mathsf{raw}$高16位（$[63:48]$，进位累积度最大）乘$K_2$后低8位回填当前面、高8位注入相邻面，低7位写入idx高半部分供下一步动态调节。初始化采用三乘积+MDS over $\mathrm{GF}(2^8)$+键相关搅拌+64步预热。纯C++14实现（GCC -O3，无SIMD）在i9-14900HX上达到5.84~GB/s。NIST SP 800-22全部通过，差分雪崩50\%、零差率0.0\%、代数度$\ge 26$，$2^{30}$对线性深测偏差$3.45\times10^{-5}$（$<3\sigma$）。
...
512位$8\times8\times8$反馈矩阵、512位（8条64位）Galois LFSR、以及16位游标idx（低9位计数+高7位存反馈态）。
...
$\mathrm{idx}$    & 16 bit & 低9位计数(0--511)，高7位存反馈态 \\
...
\midrule
$\mathrm{idx}$   & 16 bit & 低9位步计数(0--511)，高7位存反馈态 \\
...
\textbf{反馈路径}：取 $\mathsf{raw}$ 高 16 位（$[63:48]$，进位累积度最大）乘 $K_2$，低 8 位异或回当前矩阵地址并做 $\mathrm{ROTL}_8$ 行内扩散，高 8 位交叉注入相邻面同行；低 7 位同时写入 $\mathsf{idx}[15:9]$ 供下一步 TV 动态调节。面隔离天然保证批处理内无 RAW 冲突，512 步周期内每面受双重注入。实测无 $K_2$ 回填时代数度 $<7$，保留则 $\ge 26$；$2^{30}$ 对线性深测偏差 $3.45\times10^{-5} < 3\sigma$。
