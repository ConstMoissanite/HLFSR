...
\textbf{阶段三：$\mathsf{idx}$高7 位矩阵搅拌。}$\mathsf{idx\_init}$的16位全部利用：低9位为起始步游标，高7位（0--127）经线性扩散（$\mathsf{extra} + i \times \mathtt{0xB9}$）异或到全64个矩阵字节。相同密钥材料+不同高7位$\to$128种不同初始状态，等效密钥空间扩展128倍，零热路径开销。
...
\textbf{结论}：$d=21$--24 各 10 次试验全部 100\% 非零，排除代数度 $<24$。掩码乘法（mk = mask·K₂ | 1）将 8 位掩码映射为 64 位奇数乘子，每步产生不同的 GF(2) 上度 $\approx 32$ 的置换函数，256 种乘子每步变化，构成完整的非线性函数族。乘法理论峰值 $\approx 32$，当前实测边界 $\ge 24$，受限于高阶差分检验的统计分辨力（$d>24$ 时 $2^d$ 超过 $1.6\times10^7$ 元素/次试验）。
