...
\textbf{掩码乘法}：将矩阵一字节映射为64位奇数乘子（$\mathsf{mk} = \mathsf{mask}\cdot K_2 \mid 1$），以乘法非线性替代逐位选通——全8条LFSR无条件异或后经$\mathsf{mk}$加权混合，每步产生不同的GF(2)上度$\approx 32$的置换函数。与位选通方案密码学等价，但消除了SIMD阻塞点。
