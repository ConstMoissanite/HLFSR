...
\begin{abstract}
HLFSR-64是一种面向软件性能优化的流密码，采用$8\times8\times8$位反馈矩阵、8条64位Galois LFSR和9位步计数器作为状态。每步推进全部LFSR后全异或得$v_x$，经$\mathrm{ROTL}_{33}$自旋转与掩码派生的奇数乘子级联相乘产生$\mathsf{raw}$。256位输出通过单次额外乘法派生：LFSR与矩阵状态交叉后乘$K_3$，以不同旋转量生成4路乘子分别与$\mathsf{raw}$相乘，各路异或不同LFSR值消除派生相关性。反馈为$\mathsf{raw}$低16位乘$K_2$后低8位回填当前面、高8位注入相邻面。初始化采用三乘积+MDS over $\mathrm{GF}(2^8)$+键相关搅拌+64步预热。纯C++14实现（GCC -O3，无SIMD）在i9-14900HX上达到6.25~GB/s。NIST SP 800-22全部通过，差分雪崩50\%、零差率0.0\%、代数度$\ge 26$，$2^{30}$对线性深测偏差$3.45\times10^{-5}$（$<3\sigma$）。
...
\State $\mathsf{tv} \gets \mathsf{lfsr}[\mathsf{face}] \times \mathsf{matrix}[\mathsf{addr}] \times ((\mathsf{mask} \land \mathtt{0x7F}) \mid 1) \times K_3$
\For{$i \gets 0$ \textbf{to} $3$}
    \State $\mathsf{out}[i] \gets \mathsf{raw} \times \mathrm{ROTL}_{33+17i}(\mathsf{tv}) \oplus \mathsf{lfsr}_{(\mathsf{face}+2i)\bmod 8} \oplus \mathsf{curbit}^{64}$
\EndFor
...
\midrule
\textbf{HLFSR-64} & 8~MiB  & \textbf{6.25~GB/s} & 自研，纯C++14，GCC\,16.1.0 \texttt{-O3}，TV 256-bit \\
\textbf{HLFSR-64} & 64~MiB & — & 同上（待测） \\
\textbf{HLFSR-64} & 1~MiB  & — & 同上，膝点约 2~MB \\
...
\begin{enumerate}
\item \textbf{软件性能}：6.25~GB/s纯C++14（无SIMD），为ChaCha20优化C实现的10.7倍
