...
\begin{abstract}
本文提出HLFSR-64，一种面向软件性能优化的流密码算法。该算法采用$8\times8\times8$位反馈空间（512位）,9Bit计数寄存器与8条64位Galois线性反馈移位寄存器（LFSR）作为核心状态，每步将全部8条LFSR无条件异或后与掩码字节派生的奇数乘子级联相乘（$v_x \cdot \mathsf{mk} \cdot K_1$），以乘法提供非线性性与扩散。反馈路径取乘法结果低16位乘$K_2$后，低8位回填当前矩阵地址并做$\mathrm{ROTL}_8$行内扩散，高8位交叉注入相邻面同行，形成面间交叉反馈闭环。初始化阶段采用模乘预混合与$8\times8$ $\mathrm{GF}(2^8)$ 的AES-Like MDS密钥编排，再进行64步预热，通过一次状态全量更新实现了完全扩散。在实验平台（Intel Core i9-14900HX）上，纯C++14实现（GNU GCC O3,无SIMD）达到5.89~GB/s的吞吐量，为ChaCha20优化C实现的10.7倍。经NIST SP 800-22官方STS 2.1.2测试，200条1~Mbit序列全部9项有效检验通过（通过率194--200/200）。密码分析表明：差分雪崩效应达到50\%，零差分率降至0.0\%，代数度具有$\ge 26$的下界，$2^{30}$对线性深测偏差$3.45\times10^{-5}$（低于$3\sigma$噪声级$9.16\times10^{-5}$，通过检验）。

\textbf{关键词}：流密码；线性反馈移位寄存器；反馈状态机；乘性混合；MDS矩阵
\end{abstract}
...
\textbf{ARX 结构}（加-异或-循环移位）：以 Salsa20/ChaCha20 \cite{chacha} 为代表。ChaCha20 每轮对 512 位状态执行 4 次独立的列操作和 4 次对角线操作，每列涉及 4 次 32 位 ADD、XOR 和 ROL，需 20 轮（共 80 次 ADD + 80 次 XOR + 80 次 ROL）才能达到安全扩散 \cite{chacha}。ARX 天然规避查表侧信道，已标准化为 RFC~8439 并成为 TLS~1.3 强制算法。优化 C 实现吞吐约 550\,MB/s，20 轮串行依赖限制了单核性能上限，但其SIMD友好的结构使得在硬件支持的条件下性能极高。
...
HLFSR-64的总体数据流如图\ref{fig:arch}所示。系统在数据流层面维护1033位内部状态，由三部分组成：
512位$8\times8\times8$反馈矩阵、512位（8条64位）Galois LFSR、以及9位游标idx。
...
\midrule
\textbf{HLFSR-64} & 8~MiB  & \textbf{5.89~GB/s} & 自研，纯 C++14，GCC 16.1.0 \texttt{-O3}，TV 256-bit 输出 \\
\textbf{HLFSR-64} & 64~MiB & \textbf{1.87~GB/s} & 同上 \\
...
测试平台：Intel Core i9-14900HX (P-core 5.8\,GHz, 36\,MB L3), Windows 11 x64, GCC 16.1.0, \texttt{-O3 -march=native}，单线程，后台负载已控制一致。HLFSR-64 为自研实现；ChaCha20、AES-256-GCM、SM4-CBC 均取自 OpenSSL 3.4 内置实现；ChaCha20 (参考) 取自 Bernstein 原始可移植 C 代码 \cite{chacha}。受限于平台，仅支持AVX2而不支持对于Chacha20提升作用巨大的AVX512,因此此处Chacha20数据主要用于同等环境参考，不代表最高性能。
...
\quad 字节/周期 (B/cycle)   & \textbf{8.0} & 6.25 & 3.33 \\
...
从对比可见，HLFSR-64通过单指令模乘替代多轮迭代的路径，在纯软件吞吐上较为优秀（ChaCha20的3.7倍、SNOW 3G的5.8倍），同时保持了与AES-128可比的ASIC面积效率（0.28 vs 0.40 Gbps/Kgate）。主要代价是标准化程度低且缺乏第三方的公开密码分析积累。
...
\begin{enumerate}
\item \textbf{软件性能}：5.89~GB/s纯C++14（无SIMD），为ChaCha20优化C实现的10.7倍
...
\textbf{已知局限}：(1) 缺乏形式化安全归约——安全性基于设计复杂性而非已知计算困难问题；(2) 未经第三方独立密码分析；(3) 代数度实测$\ge 26$，乘法器理论峰值$\sim 32$，$d>24$ 时 $2^d$ 爆炸限制统计分辨力；(4) 64$\times$64全乘法器占ASIC面积60--78\%，资源受限场景下需架构优化。
