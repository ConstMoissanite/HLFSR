...
\begin{abstract}
本文提出HLFSR-64，一种面向软件性能优化的流密码算法。该算法采用$8\times8\times8$位反馈空间（512位）与8条64位Galois线性反馈移位寄存器（LFSR）作为核心状态，每步从矩阵读取一个字节作为8位掩码选通LFSR参与输出异或运算，经64位模乘进行非线性混合。反馈路径取乘法结果的低16位乘$K_2$，低8位回填矩阵、高8位注入LFSR的对应字节，形成矩阵-LFSR交叉扩散。初始化阶段采用模乘预混合与$8\times8$ $\mathrm{GF}(2^8)$ MDS密钥编排，再进行64步预热，实现了完全扩散。在实验平台（Intel Core i9-14900HX）上，纯C++14实现（无SIMD）达到2.03~GB/s的吞吐量，为ChaCha20优化C实现的3.7倍。经NIST SP 800-22官方STS 2.1.2测试，200条1~Mbit序列全部9项有效检验通过（通过率194--200/200）。密码分析表明：差分雪崩效应达到50\%，零差分率降至0.0\%，代数度$\ge 24$，随机线性掩码$2^{30}$对深测偏差$3.45\times10^{-5}$（低于$3\sigma$噪声级$9.16\times10^{-5}$，通过检验）（远低于$3\sigma$噪声级0.134）。
...
\textbf{反馈闭环}：输出低 8 位经 $K_2$ 乘法后异或回矩阵，再经 $\mathrm{ROTL}_8$ 按列号旋转，形成"状态 $\to$ 掩码 $\to$ 异或 $\to$ 乘法 $\to$ 回填 $\to$ 状态"的自指闭环。该机制使状态更新函数每步变化，消除了固定轮函数结构下在滑动攻击中的可利用性。
...
\textbf{位广播说明}：记号 $(x \gg 63)^{64}$ 表示提取 $x$ 的最高位（0 或 1），重复 64 次构成全 0 或全 1 掩码字。该掩码与多项式常量逐位与，实现无分支条件异或——若 MSB=1 则异或常量，否则不变。同理 $((\mathsf{mask} \gg i) \land 1)^{64}$ 将 $\mathsf{mask}$ 的第 $i$ 位广播为 LFSR 选通掩码，$(\mathsf{mask} = 0)^{64}$ 用于零掩码回退的条件选择。这三处均不依赖 $if$ 分支，实际实现中利用补码取负指令 \cite{chacha} 在常数时间内完成广播。
...
\EndFor
\State $\mathsf{mz} \gets (\mathsf{mask} = 0)^{64}$ \Comment{全 0/全 1 掩码，常数时间无分支选择}
\State $\mathsf{raw} \gets ((v_x \land \lnot\mathsf{mz}) \lor (\mathsf{lfsr}[\mathsf{idx} \land 7] \land \mathsf{mz})) \times K_1$
\Comment{$K_1 = \mathtt{0x9E3779B97F4A7C15}$，$\mathsf{mask}=0$ 回退（概率 $1/256$）}
...
\State $\mathsf{lfsr}[\mathsf{idx} \land 7] \gets \mathsf{lfsr}[\mathsf{idx} \land 7] \oplus ((\mathsf{fb} \gg 8) \ll ((\mathsf{idx} \gg 3) \land 7) \times 8)$
\Comment{$K_2 = \mathtt{0xBF58476D1CE4E5B9}$；16-bit反馈：低8位回填矩阵，高8位注入LFSR}
...
8个多项式的非零系数个数（即 Hamming 权重）在 13--15 范围内，属中等偏高密度，以补偿每条 LFSR 因 mask 选通造成的高选中率（期望约 $50\%$）。各多项式互素，组合周期为 $\prod_{i=0}^{7}(2^{64}-1)$。全部多项式由工具随机采样并经不可约性与本原性双重验证筛选得到。
...
反馈路径取乘法结果低 16 位乘以 $K_2$：低 8 位异或回矩阵对应字节并做 $\mathrm{ROTL}_8$ 行内扩散，高 8 位注入 $\mathsf{lfsr}[\mathsf{idx} \land 7]$ 的对应字节位。LFSR 注入点随 idx 轮转——每条 LFSR 每 512 步被击中 64 次、每个字节位 8 次。实测表明：无 $K_2$ 回填时代数度 $<7$，仅 8 位回填时 $\ge 22$，16 位交叉反馈将度边界推至 $\ge 23$。$K_2$ 乘法在反馈回路中累积代数深度，注入 LFSR 侧使原本纯线性的 LFSR 递推获得非线性扰动，弥补了输出乘法仅作用于线性 XOR 和的局限。
...
采用高阶差分方法 \cite{biham,courtois}：对维度$d=1..16$的随机仿射子空间进行检验。若子空间所有$2^d$个元素的输出异或为零，则代数度$<d$。$d\le 11$时4--100次试验，$d=12$--16各10次试验。
...
\textbf{21--24} & \textbf{10} & \textbf{100\%} & \textbf{度 $\ge 24$} \\
\bottomrule
\end{tabular}
\caption{代数度检验结果}
\label{tab:degree}
...
\textbf{结论}：$d=21$--24 各 10 次试验全部 100\% 非零，排除代数度 $<24$。掩码乘法（mk = mask·K₂ | 1）将 8 位掩码映射为 64 位奇数乘子，每步产生不同的 GF(2) 上度 $\approx 32$ 的置换函数，与位选通方案等价但消除了 SIMD 阻塞点。乘法理论峰值 $\approx 32$，当前实测边界 $\ge 24$，受限于高阶差分检验的统计分辨力（$d>24$ 时 $2^d$ 超过 $1.6\times10^7$ 元素/次试验）。
...
初筛阶段（5000掩码 $\times$ 500对）最大偏差0.086，全部掩码低于$3\sigma$噪声级0.134。对偏差最大的掩码（\#4227, Hamming权重252/512）进行$10^7$对上界筛查（bias $4.8\times10^{-5}$，远低于$10^{-4}$阈值）后，全量$2^{30}$对深测收敛于$3.45\times10^{-5}$，低于对应$3\sigma$噪声级$9.16\times10^{-5}$——在统计上无法区分于零，通过检验。
...
\item \textbf{密码分析}：差分雪崩50\%，零差分率0.0\%，代数度$\ge 24$，$2^{30}$对线性深测偏差$3.45\times10^{-5}$（低于$3\sigma$，通过）
