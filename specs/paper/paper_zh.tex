...
\title{HLFSR-64：一种基于掩码乘法与乘性混合的\\高吞吐软件流密码算法}
\author{Pt\_195\\[2mm]
...
\textbf{掩码乘法}：将矩阵一字节映射为64位奇数乘子（$\mathsf{mk} = \mathsf{mask}\cdot K_2 \mid 1$），以乘法非线性替代逐位选通——全8条LFSR无条件异或后经$\mathsf{mk}$调味，每步产生不同的GF(2)上度$\approx 32$的置换函数。与位选通方案密码学等价，但消除了SIMD阻塞点。

\textbf{双乘分离}：输出端$K_1$确保端到端统计质量，回填端$K_2$在反馈回路中累积代数深度。$\mathsf{vx}$进乘法前经$\mathrm{ROTL}_{33}$自旋转混合打破LSb线性依赖。实测代数度$\ge 24$，$2^{30}$对线性深测通过检验。
...
\textbf{位广播说明}：记号 $(x \gg 63)^{64}$ 提取 $x$ 的最高位并重复 64 次构成全 0/全 1 掩码，用于 Galois LFSR 的无分支条件异或 \cite{chacha}。$\mathsf{curbit}^{64}$ 同理将单比特广播为 64 位全 0/全 1 输出翻转字。$\mathsf{mk} = (\mathsf{mask} \times K_2) \mid 1$ 通过乘法将 8 位掩码映射为 64 位奇数乘子（$\mathsf{mask}=0$ 时 $\mathsf{mk}=1$ 恒等），替代传统的逐位选通与零掩码回退，无需额外分支。
...
\EndFor
\State $v_x \gets v_x \oplus ((v_x \ll 33) \lor (v_x \gg 31))$
\Comment{ROTL$_{33}$自旋转打破LSb线性，进位链放大}
...
每步从矩阵读取1字节 $\mathsf{mask}$，通过乘法映射为 64 位奇数乘子 $\mathsf{mk} = (\mathsf{mask} \times K_2) \mid 1$。全部8条LFSR无条件参与异或生成 $v_x$，$v_x$ 经 $\mathrm{ROTL}_{33}$ 自旋转打破LSb线性依赖后，与 $\mathsf{mk}$ 和 $K_1$ 级联相乘：$\mathsf{raw} = v_x \times \mathsf{mk} \times K_1$。$\mathsf{mk}$ 随矩阵内容每步变化（256种不同乘子），产生了256种不同的 GF(2) 上度 $\approx 32$ 的置换函数——与位选通方案在函数族大小和代数复杂度上等价，但数学结构更简洁。
...
采用高阶差分方法 \cite{biham,courtois}：对维度$d=1..24$的随机仿射子空间进行检验。若子空间所有$2^d$个元素的输出异或为零，则代数度$<d$。$d\le 11$时4--100次试验，$d=12$--24各10次试验。
...
代数攻击 \cite{courtois} & 1033状态位 $\times$ 度$\ge 24$ $\times$ 掩码乘子变化 & $>2^{200}$ (XL/XSL) \\
