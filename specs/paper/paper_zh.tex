...
\textbf{估算方法}：门数基于模块级手工估算（64×64 Dadda乘法器 $\approx$ 7K 门、8×64b LFSR寄存器 $\approx$ 3K 门、控制逻辑+矩阵RAM $\approx$ 1K 门）；频率基于乘法器关键路径（$\sim$6--8 FO4 $\approx$ 1.0--1.5ns @ 7nm），经验降额至 800 MHz。以上均未经实际逻辑综合，误差 ±30--50\% 可预期。ChaCha20 和 AES-128 的对照数据引自已发表文献的 7nm 缩放估算。
