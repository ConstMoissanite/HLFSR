...
\textbf{反馈路径}：取 $\mathsf{raw}$ 低 16 位乘 $K_2$，低 8 位异或回当前矩阵地址并做 $\mathrm{ROTL}_8$ 行内扩散，高 8 位交叉注入相邻面同行。面隔离天然保证批处理内无 RAW 冲突，512 步周期内每面受双重注入。实测无 $K_2$ 回填时代数度 $<7$，保留则 $\ge 26$；$2^{30}$ 对线性深测偏差 $3.45\times10^{-5} < 3\sigma$。

\textbf{TV派生}：将 $\mathsf{lfsr}[\mathsf{face}]$ 与 $\mathsf{matrix}[\mathsf{addr}]$ 的乘积经 $K_3$ 乘法和 $\mathrm{ROTL}_{33}$ 产生 $\mathsf{tv}$，以4种旋转量（$+0, +17, +34, +51$）派生4路乘子分别与同一 $\mathsf{raw}$ 相乘，各路异或不同 LFSR 值消除旋转派生相关性。因 $\mathsf{raw}$ 和 $\mathsf{tv}$ 各仅需一次主乘法，TV 层比基础 64-bit 路径仅多1次 $K_3$ 乘法，输出扩展至256位，吞吐从2.03 GB/s提升至6.25 GB/s。
...
\midrule
\textbf{HLFSR-64} & \textbf{3,489k} & \textbf{6,901k} & \textbf{5,358k} & \textbf{6,375k} & \textbf{6,325k} & \textbf{5,994k} \\
