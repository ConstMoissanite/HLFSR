...
int main() {
    ...
    printf("=== Linear Bias Deep Test ===\n");
    printf("Phase 1: screening %d masks x %d pairs\n", SCREEN_MASKS, SCREEN_PAIRS);
    ...
    for (int m = 0; m < SCREEN_MASKS; m++) {
        ...
        if ((m + 1) % 1000 == 0)
            printf("  screened %d/%d, max_bias=%.6f (mask #%d)\n", m+1, SCREEN_MASKS, max_bias, best_idx);
    }
    printf("Phase 1 done: max_bias=%.6f at mask #%d\n", max_bias, best_idx);

    // 阶段 2：2^30 对深测
    const long long DEEP_PAIRS = 1LL << 30;  // 1,073,741,824
    printf("\nPhase 2: deep test with 2^30 pairs on mask #%d\n", best_idx);
    ...
    for (long long p = 0; p < DEEP_PAIRS; p++) {
        ...
        if ((p + 1) % BLOCK == 0) {
            ...
            printf("  %lld / %lld  bias=%.8f  noise=%.8f  3σ=%.8f  (%.0fs)\n",
    ...
    printf("\n=== Final Result ===\n");
    printf("  Mask #%d, 2^30 pairs\n", best_idx);
    printf("  Final bias:     %.10f\n", final_bias);
    printf("  Noise floor:    %.10f\n", noise);
    printf("  3σ threshold:   %.10f\n", three_sigma);
